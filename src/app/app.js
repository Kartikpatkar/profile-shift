...
const els = {
  ...
  sysPermCount: document.getElementById('sysPermCount'),
  apexCount: document.getElementById('apexCount'),
  vfCount: document.getElementById('vfCount'),
  tabCount: document.getElementById('tabCount'),
  rtCount: document.getElementById('rtCount'),
...
function debugLog(label, value) {
  ...
  // Don't render large JSON blobs in the UI.
  if (els.debug) {
    els.debug.textContent = 'Debug output is in the DevTools Console.';
  }
}
...
function updateCountsFromModel() {
  ...
  els.profileName.textContent = String(latestModel?.profileName || latestModel?.debug?.profileFullName || '—');
  els.objCount.textContent = String(counts.objects);
  els.fieldCount.textContent = String(counts.fields);
  els.sysPermCount.textContent = String(counts.sysPerms);
  els.apexCount.textContent = String(counts.apex);
  els.vfCount.textContent = String(counts.vf);
  els.tabCount.textContent = String(counts.tabs);
  els.rtCount.textContent = String(counts.recordTypes);
}
...
function filterRowsForSection(sectionKey, rows, q) {
  ...
  switch (sectionKey) {
    case 'objects':
      return (rows || []).filter((r) => includesQuery(r?.object, query));
    case 'fields':
      return (rows || []).filter((r) => includesQuery(r?.field, query));
    case 'system':
      return (rows || []).filter((r) => includesQuery(r?.name, query));
    case 'apex':
      return (rows || []).filter((r) => includesQuery(r?.apexClass, query));
    case 'vf':
      return (rows || []).filter((r) => includesQuery(r?.apexPage, query));
    case 'tabs':
      return (rows || []).filter((r) => includesQuery(r?.tab, query));
    case 'recordTypes':
      return (rows || []).filter((r) => includesQuery(r?.recordType, query));
    default:
      return rows;
  }
}
...
function renderEditor() {
  ...
  wrap.appendChild(el('div', { class: 'tableTitle', text: titleText }));
