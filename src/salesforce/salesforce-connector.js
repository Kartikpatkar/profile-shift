...
class SalesforceConnector {
    ...
    constructor(options = {}) {
        ...
        this.pinnedTabId = Number.isInteger(options.pinnedTabId) ? options.pinnedTabId : null;
    }
    ...
    async checkAuth(options = {}) {
        try {
            ...
            const pinnedTabId = Number.isInteger(options.pinnedTabId)
                ? options.pinnedTabId
                : this.pinnedTabId;
            ...
            if (pinnedTabId) {
                ...
                try {
                    ...
                    if (pinnedTab && this._isSalesforceUrl(pinnedTab.url)) {
                        const pinnedHost = new URL(pinnedTab.url).hostname;
                        if (!this._isLoginOrTestHost(pinnedHost)) {
                            const result = await this._checkTabForSession(pinnedTab);
                ...
                this._cache = { org: notAuthPinned, timestamp: Date.now() };
                this._notifyAuthChange(notAuthPinned);
                return notAuthPinned;
            }
